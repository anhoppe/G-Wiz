...
namespace Gwiz.UiControl.WinUi3
{
    internal class GraphDrawer
    {
        ...
        private void DrawEdges()
        {
            if (Edges != null)
            {
                foreach (var edge in Edges)
                {
                    ...
                    Draw.DrawLine(edge.FromPosition, modifiedEndingPosition);
                }
            }
        }
        ...
        private Point DrawEdgeEnding(IEdge edge)
        {
            ...
            Draw.DrawLine(edge.ToPosition, arrowHead1);
            ...
            Draw.DrawLine(edge.ToPosition, arrowHead2);

            if (edge.Ending == Ending.ClosedArrow)
            {
                // 'Close' the arrow
                Draw.DrawLine(arrowHead1, arrowHead2);
            ...
            else if (edge.Ending == Ending.Rhombus)
            {
                ...
                Draw.DrawLine(arrowHead1, rhombusPoint);
                Draw.DrawLine(arrowHead2, rhombusPoint);
        ...
        private void DrawNodeGrid(INode node)
        {
            ...
            for (int x = 0; x < grid.Cols.Count; x++)
            {
                for (int y = 0; y < grid.Rows.Count; y++)
                {
                    ...
                    Draw.DrawText(grid.FieldText[x][y], new Point((int)xText, (int)yText), node.LineColor, TextFormat);
                }
            }
        }
